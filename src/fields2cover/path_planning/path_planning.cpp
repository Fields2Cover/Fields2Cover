...
namespace f2c::pp
{
    ...
    F2CPath PathPlanning::planPath(const F2CRobot &robot, const F2CSwaths &swaths, TurningBase &turn)
    {
        ...
        if (swaths.size() > 1)
        {
            for (size_t i = 0; i < swaths.size() - 1; ++i)
            {
                ...
                F2CPath turn_path = turn.createTurn(robot, swaths[i].endPoint(), swaths[i].getOutAngle(), swaths[i + 1].startPoint(), swaths[i + 1].getInAngle());
        ...
        const F2CRobot &robot, const F2CPoint &p1, double ang1, const F2CMultiPoint &mp, const F2CPoint &p2, double ang2, TurningBase &turn)
    {
        ...
        {
            path += turn.createTurn(robot, v_con[i - 1].first, v_con[i - 1].second, v_con[i].first, v_con[i].second);
        }
        return path;
    }
    ...
    {
        double x, y, ang, k;
        end_of_clothoid(0.0, 0.0, 0.0, 0.0, robot.getMaxDiffCurv(), 1.0, robot.getMaxCurv() / robot.getMaxDiffCurv(), &x, &y, &ang, &k);
