...
F2CPath PathPlanning::planPath(const F2CRobot& robot, const F2CRoute& route,
    TurningBase& turn, bool prevent_removing_connections) {
  F2CPath path;
  for (size_t i = 0; i < route.sizeVectorSwaths(); ++i) {
    auto prev_swaths = (i > 0) ? route.getSwaths(i - 1) : F2CSwaths();
    path += planPathForConnection(robot, prev_swaths, route.getConnection(i),
        route.getSwaths(i), turn, prevent_removing_connections);
    path += planPath(robot, route.getSwaths(i), turn);
  }
  if (route.sizeConnections() > route.sizeVectorSwaths()) {
    path += planPathForConnection(robot, route.getLastSwaths(),
        route.getLastConnection(), F2CSwaths(), turn,
        prevent_removing_connections);
  }
  return path;
}

F2CPath PathPlanning::planPath(
    const F2CRobot& robot, const F2CSwaths& swaths, TurningBase& turn) {
  F2CPath path;
  if (swaths.size() > 1) {
    for (size_t i = 0; i < swaths.size() - 1; ++i) {
      path.appendSwath(swaths[i], robot.getCruiseVel());
      F2CPath turn_path =
          turn.createTurn(robot, swaths[i].endPoint(), swaths[i].getOutAngle(),
...
F2CPath PathPlanning::planPathForConnection(const F2CRobot& robot,
  ...
  return planPathForConnection(
      robot, p1, ang1, mp, p2, ang2, turn, prevent_removing_connections);
}

F2CPath PathPlanning::planPathForConnection(const F2CRobot& robot,
    const F2CPoint& p1, double ang1, const F2CMultiPoint& mp,
    const F2CPoint& p2, double ang2, TurningBase& turn,
    bool prevent_removing_connections) {
  auto v_con = simplifyConnection(
      robot, p1, ang1, mp, p2, ang2, prevent_removing_connections);

  F2CPath path;
  for (int i = 1; i < v_con.size(); ++i) {
    path += turn.createTurn(robot, v_con[i - 1].first, v_con[i - 1].second,
...
double PathPlanning::getSmoothTurningRadius(const F2CRobot& robot) {
  ...
  end_of_clothoid(0.0, 0.0, 0.0, 0.0, robot.getMaxDiffCurv(), 1.0,
      robot.getMaxCurv() / robot.getMaxDiffCurv(), &x, &y, &ang, &k);
...
std::vector<std::pair<F2CPoint, double>> PathPlanning::simplifyConnection(
    const F2CRobot& robot, const F2CPoint& p1, double ang1,
    const F2CMultiPoint& mp, const F2CPoint& p2, double ang2,
    bool prevent_removing_connections) {
  ...
  if (!prevent_removing_connections &&
      ((p1.distance(p2) < 6.0 * safe_dist) || mp.size() < 2)) {
  ...
  for (int i = 1; i < vp.size() - 1; ++i) {
    ...
    if (prevent_removing_connections ||
        (p_in.distance(path.back().first) > 3.0 * safe_dist &&
    ...
    if (prevent_removing_connections ||
        (p_out.distance(vp[i + 1]) > 3.0 * safe_dist &&
