...
TEST(fields2cover_custom_order, incomplete_order) {
  const int n = 11;
  F2CSwaths swaths;
  for (int i = 1; i < n; ++i) {
    swaths.emplace_back(F2CLineString({F2CPoint(0, i), F2CPoint(1, i)}), i, i-1);
  ...
  f2c::rp::CustomOrder swath_sorter(swaths, {9,0,8,1,7,2,6,3});
  
  EXPECT_ANY_THROW(swath_sorter.genSortedSwaths()); 

}

TEST(fields2cover_custom_order, bad_order) {
  const int n = 11;
  F2CSwaths swaths;
  for (int i = 1; i < n; ++i) {
    swaths.emplace_back(F2CLineString({F2CPoint(0, i), F2CPoint(1, i)}), i, i-1);
  ...
  f2c::rp::CustomOrder swath_sorter(swaths, {9,1,8,1,7,2,6,3,5,4});

  EXPECT_ANY_THROW(swath_sorter.genSortedSwaths());

}

TEST(fields2cover_custom_order, out_of_range) {
  const int n = 11;
  F2CSwaths swaths;
  for (int i = 1; i < n; ++i) {
    swaths.emplace_back(F2CLineString({F2CPoint(0, i), F2CPoint(1, i)}), i, i-1);
  ...
  f2c::rp::CustomOrder swath_sorter(swaths, {10,0,8,1,7,2,6,3,5,4});
