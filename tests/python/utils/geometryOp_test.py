...
def test_fields2cover_utils_GeometryOp_getAngleRandom():
  rand = f2c.Random();
  rand_val = 0.0;
  two_pi = 2.0 * math.pi;
...
def test_fields2cover_utils_GeometryOp_getRandomDouble():
  rand = f2c.Random();
  N = 10000;
  sum_val = 0.0;
  for i in range(N):
...
def test_fields2cover_utils_GeometryOp_getExpRandom():
  rand = f2c.Random();
  rand_val = 0.0;
  for i in range(100):
...
def test_fields2cover_utils_GeometryOp_getAngleFromPoints():
  ...
  near(p1.getAngleFromPoints(p1), 0, 0.01);
  near(p1.getAngleFromPoints(p4), pi, 0.01);
  near(p1.getAngleFromPoints(p3), pi / 4.0, 0.01);
  near(p1.getAngleFromPoints(p2), pi / 2.0, 0.01);
  near(p2.getAngleFromPoints(p1), 3 * pi / 2.0, 0.01);
  near(p1.getAngleFromPoints(p5), 20.0 * degree, 0.01);
...
"""
def test_fields2cover_utils_GeometryOp_genRandField():
  rand = f2c.Random();
  field = rand.generateRandField( 4, 1e2);
  near(field.field.getArea(), 1e2 , 1e-6);
...
def test_fields2cover_utils_GeometryOp_isConvex():
  rand = f2c.Random(4);
  n = 100;
  field = rand.generateRandField( 3, 1e2);
