...
TEST(fields2cover_utils_parser, file_not_found) {
  F2CFields fields;
  EXPECT_THROW(f2c::Parser::importGml("file_not_found.xml", fields),
...
TEST(fields2cover_utils_parser, import_strips_json) {
  F2CStrips strips =
      f2c::Parser::importStripsJson(std::string(DATA_PATH) + "strips.geojson");
...
TEST(fields2cover_utils_parser, import_swaths_json) {
  F2CSwaths swaths =
      f2c::Parser::importSwathsJson(std::string(DATA_PATH) + "swaths.geojson");
...
TEST(fields2cover_utils_parser, import_json_from_string) {
  // Need more complex tests
  F2CFields fields;
  std::ifstream f(std::string(DATA_PATH) + "test.json");
  std::stringstream buffer;
  buffer << f.rdbuf();
  f2c::Parser::importJsonFromString(buffer.str(), fields);
  auto field = fields[0];
  field.setEPSGCoordSystem(4326);
...
TEST(fields2cover_utils_parser, import_json_two_cells_from_string) {
  F2CFields fields;
  std::ifstream f(std::string(DATA_PATH) + "test_2_cell.json");
  std::stringstream buffer;
  buffer << f.rdbuf();
  f2c::Parser::importJsonFromString(buffer.str(), fields);
  EXPECT_EQ(fields.size(), 2);
...
TEST(fields2cover_utils_parser, import_cell_json_from_string) {
  F2CCell cell;
  std::ifstream f(std::string(DATA_PATH) + "test.json");
  std::stringstream buffer;
  buffer << f.rdbuf();
  cell = f2c::Parser::importCellJsonFromString(buffer.str());
  EXPECT_EQ(cell.size(), 1);
  EXPECT_EQ(cell.getGeometry(0).size(), 20);
  EXPECT_GT(cell.area(), 0);
}

TEST(fields2cover_utils_parser, import_strips_json_from_string) {
  F2CStrips strips;
  std::ifstream f(std::string(DATA_PATH) + "strips.geojson");
  std::stringstream buffer;
  buffer << f.rdbuf();
  strips = f2c::Parser::importStripsJsonFromString(buffer.str());
  EXPECT_EQ(strips.size(), 3);
  EXPECT_EQ(strips[0].getName(), "Winter barley");
  EXPECT_EQ(strips[1].getName(), "Potato (late)");
  EXPECT_GT(strips[0].getCell().area(), 0);
...
TEST(fields2cover_utils_parser, import_swaths_json_from_string) {
  F2CSwaths swaths;
  std::ifstream f(std::string(DATA_PATH) + "swaths.geojson");
  std::stringstream buffer;
  buffer << f.rdbuf();
  swaths = f2c::Parser::importSwathsJsonFromString(buffer.str());
  EXPECT_EQ(swaths.size(), 3);
  EXPECT_EQ(swaths[0].getWidth(), 0.3);
  EXPECT_EQ(swaths[1].getWidth(), 0.3);
  EXPECT_EQ(swaths[0].getId(), 44);
...
TEST(fields2cover_utils_parser, import_json_ring_from_string) {
  F2CFields fields;
  std::ifstream f(std::string(DATA_PATH) + "test_ring.json");
  std::stringstream buffer;
  buffer << f.rdbuf();
  f2c::Parser::importJsonFromString(buffer.str(), fields);
  EXPECT_EQ(fields.size(), 1);
