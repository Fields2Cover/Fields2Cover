...
TEST(fields2cover_types_cells, carveSharedBordersOnlyBetweenTouchingCells) {
...
TEST(fields2cover_types_cells, carveSharedBordersLeavesLoneCellsAlone) {
  ...
  EXPECT_NEAR(apart.carveSharedBorders(2.0).area(), apart.area(), 1e-3);

  // A single cell has no neighbour at all.
  F2CCells one {left};
  EXPECT_NEAR(one.carveSharedBorders(2.0).area(), one.area(), 1e-3);
}

TEST(fields2cover_types_cells, carveSharedBordersIgnoresCornerContact) {
  ...
  EXPECT_NEAR(cells.carveSharedBorders(2.0).area(), cells.area(), 1e-6);
}

TEST(fields2cover_types_cells, carveSharedBordersOnPartiallyTouchingEdge) {
...
TEST(fields2cover_types_cells, carveSharedBordersKeepsBothHalvesOfASplitCell) {
  ...
  F2CCells carved = cells.carveSharedBorders(6.0);
...
TEST(fields2cover_types_cells, carveSharedBordersLeavesNoHairlineSpur) {
  ...
  F2CCells carved = decomp.decompose(
